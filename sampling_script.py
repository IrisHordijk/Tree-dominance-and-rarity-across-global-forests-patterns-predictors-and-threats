...
compositeToUse = ee.Image('users/laubert/DominanceAndRarity/AllCombined')

# !! FeatureCollection to sample
points = ee.FeatureCollection('users/laubert/DominanceAndRarity/DominanceRarityData')
...
def extract_and_write_grid(n, grids, points, region):
    ...
    if len(results) > 0:
        with open("data/sampled_data/sampled_%d.csv" % n, "w") as f:
            f.write(",".join(BANDS))
            f.write("\n")
...
if __name__ == '__main__':
    ...
    terra_poly = compositeToUse.select(0).toInt().reduceToVectors(
                        geometry= unboundedGeo,
                        crs= compositeToUse.select(0).projection(),
                        scale= compositeToUse.select(0).projection().nominalScale().getInfo(),
